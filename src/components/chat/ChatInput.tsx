...
export default function ChatInput({
  ...
  return (
    <div className="p-2">
      {isRecording && (
        <div className="mb-2 px-3 py-2 bg-red-50 rounded-lg flex items-center gap-2 text-sm animate-pulse">
          <div className="w-3 h-3 bg-red-600 rounded-full"></div>
          <span className="text-red-900 font-medium">Recording: {Math.floor(recordingTime / 60)}:{(recordingTime % 60).toString().padStart(2, '0')}</span>
        </div>
      )}
        ...
        <div className="flex-1 relative bg-gray-100 rounded-2xl focus-within:bg-white focus-within:ring-1 focus-within:ring-gray-300 transition">
          ...
          <div className="absolute right-1.5 bottom-1.5 flex items-center gap-0.5">
            ...
            <button
              ...
              disabled={disabled}
              className={`p-1.5 hover:bg-gray-200 rounded-full transition disabled:opacity-50 ${isRecording ? 'bg-red-100 animate-pulse' : ''}`}
